class Response {
	constructor(contestant, round) {
		this.name = contestant.name;
		this.entry = contestant["response" + round];
		this.rank = contestant["rank" + round];
		this.score = contestant["average" + round];
		this.stdev = contestant["std_dev" + round];
...
let prize_rank = 0;
let safe_rank = 0;
function load_episode_data(ep){
	prize_rank = prize_ranks[ep];
	safe_rank = safe_ranks[ep];
	responses = [];
	contestants = contestants_master.filter((c) => c["rank" + ep] <= rr_contestants[ep]);
	for (let i = 0; i < contestants.length; i++){
		responses.push(new Response(contestants[i], ep));
	}
	...
	var data = [trace1];
	Plotly.newPlot(T, data);
    let table_body = document.getElementById("categories");
    table_body.textContent = "";
	document.getElementById("footer-all").innerText = responses.length;
	document.getElementById("footer-avg").innerText = to_percentage(score_total / responses.length);
	document.getElementById("footer-prize-count").innerText = prize_rank;
	document.getElementById("footer-safe-count").innerText = safe_rank - prize_rank;
	document.getElementById("footer-death-count").innerText = responses.length - safe_rank;
	document.getElementById("footer-prize-share").innerText = to_percentage(prize_rank / responses.length);
	document.getElementById("footer-safe-share").innerText = to_percentage((safe_rank - prize_rank) / responses.length);
	document.getElementById("footer-death-share").innerText = to_percentage((responses.length - safe_rank) / responses.length);
}
load_episode_data(total_eps);

var TESTER = document.getElementById('tester');

let ep_select = document.getElementById("ep_select");
ep_select.addEventListener("change", set_episode);
function set_episode(){
	load_episode_data(Number(ep_select.value));
}
